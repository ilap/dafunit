...
namespace Daf.UnitTest {
    ...
    public class Assert : GLib.Object {
        ...
        private static void throw_unexpected_value( string info, string debug ) throws AssertError
        {
            throw new AssertError.UNEXPECTED_VALUE( info +  ( debug.length > 0 ? @" : $(debug)" : "" ) );
        }
         ...
         **/
        public static void are_equal<T>( T expected, T actual, string debug = "" ) throws AssertError {
            if ( expected != actual ) {
                var msg = @"$(typed_value_to_string(expected)) != $(typed_value_to_string(actual))";
                throw_unexpected_value( @"Actual value is not the same as the expected one: $(msg)", debug );
            }
        }
         ...
         **/
        public static void are_not_equal<T>( T not_expected, T actual, string debug = "" ) throws AssertError
        {
            if ( not_expected == actual )
            {
                var msg = "$(typed_value_to_string(expected)) == $(typed_value_to_string(actual))";
                throw_unexpected_value( @"Actual value is the same as the not expected one: $(msg)", debug );
            }
        }
         ...
         **/
        public static void is_true ( bool actual, string debug = "" ) throws AssertError
        {
            if ( !actual )
                throw_unexpected_value( "Supplied value is not true", debug );
        }
         ...
         * @param message Text message to append to error message when assert failed
         **/
        public static void is_false ( bool actual, string debug = "" ) throws AssertError
        {
            if ( actual )
                throw_unexpected_value( "Supplied value is not false", debug );
        }
         ...
         **/
        public static void fail ( string debug ) throws AssertError
        {
            throw new AssertError.UNEXPECTED_STATE ( debug );
        }
         ...
         *                is not thrown.
         **/
        public static void should_throw_async( AsyncBegin fbegin, AsyncFinish ffinish, string domain, string debug = "" ) throws AssertError
        {
            try
            {
                if ( !wait_for_async( 200, fbegin, ffinish ) )
                    throw_unexpected_value( "Execution of async method didn't returns the expected value", debug );
            }
            catch ( GLib.Error err )
            {
                if ( err.domain.to_string() != domain )
                    throw_unexpected_value( @"Didn't receive the expected exception of type $domain", debug );
                return;
            }
